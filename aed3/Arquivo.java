...
public class Arquivo<T extends EntidadeArquivo> {
    ...
    public List<T> readAll() throws Exception {
        ...
        arquivo.seek(TAMANHO_CABECALHO); // Skip the header
        while (arquivo.getFilePointer() < arquivo.length()) {
            byte lapide = arquivo.readByte();
            short tamanho = arquivo.readShort();
            if (lapide == ' ') { // Only read active records
                byte[] dados = new byte[tamanho];
                arquivo.read(dados);
                T entidade = construtor.newInstance();
                entidade.fromByteArray(dados);
                allEntities.add(entidade);
            } else {
                arquivo.skipBytes(tamanho); // Skip deleted records
            }
        }
        return allEntities;
    }
